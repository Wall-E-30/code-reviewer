...
async def run_task(task_id):
    ...
    while step_idx <= 5:
        ...
        try:
            ...
            # Prevent sum from ever being exactly 0.0 if it crashes on the very first try
            fallback = 0.01 if not total_rewards else 0.00
            print(f"[STEP] step={step_idx} action=error reward={fallback:.2f} done=true error={str(e)}", flush=True)
            total_rewards.append(fallback)
            break
            
    success = float(round(min(max(sum(total_rewards), 0.01), 0.99), 2))
